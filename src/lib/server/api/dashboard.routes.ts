	...
	public routes() {
		return this.app
			...
			.get('/', async (c) => {
				...
				try {
					const dashboards =
						await this.dashboardService.getDashboardsForOrganization(organizationId);
			...
			.put('/:id', zValidator('json', updateDashboardSchema), async (c) => {
				const organizationId = c.var.user!.activeOrganizationId!;
				const id = c.req.param('id');
				const validatedData = c.req.valid('json');
			...
			.post('/:dashboardId/items', zValidator('json', insertDashboardItemSchema), async (c) => {
				const organizationId = c.var.user!.activeOrganizationId!;
			...
			.put('/items/:itemId', zValidator('json', updateDashboardItemSchema), async (c) => {
				const organizationId = c.var.user!.activeOrganizationId!;
			...
			.post('/items/:itemId/refresh', async (c) => {
				const organizationId = c.var.user!.activeOrganizationId!;
				const itemId = c.req.param('itemId');
				try {
...
export type DashboardRoutesType = ReturnType<DashboardRoutes['routes']>;
